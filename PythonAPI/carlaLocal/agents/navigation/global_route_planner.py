...
class GlobalRoutePlanner:
    ...
    def _make_avoid_graph(self, forbidden_areas):
        """
        forbidden_areas: List[tuple[carla.Location, float]]
        每个元素是 (圆心, 半径)，单位用 m（和 CARLA 坐标一致）
        返回一个新的 DiGraph，把这些区域里的节点和边都删掉
        """
        import math
        import copy

        # 复制一份图，不动原始 self._graph
        G = self._graph.copy()

        def in_any_forbidden(x, y, z):
            for center, r in forbidden_areas:
                dx = x - center.x
                dy = y - center.y
                # 通常忽略 z，只看平面距离
                if dx * dx + dy * dy <= r * r:
        ...
        for n, data in list(G.nodes(data=True)):
            x, y, z = data['vertex']
            if in_any_forbidden(x, y, z):
                G.remove_node(n)

        # 2) 删掉路径穿过圆的边（节点虽然在外面，但中间 path 经过禁区）
        for u, v, data in list(G.edges(data=True)):
            ...
            for wp in path:
                ...
                if in_any_forbidden(loc.x, loc.y, loc.z):
                    remove_edge = True
                    break

            if remove_edge and G.has_edge(u, v):
                G.remove_edge(u, v)

        return G

    def trace_route_avoid_areas(self, origin, destination, forbidden_areas):
        """
        origin, destination: carla.Location
        forbidden_areas: List[tuple[carla.Location, float]]

        和 trace_route 用法一样，只是额外会绕开若干圆形禁区
        """
        # 先构造过滤后的图
        avoid_graph = self._make_avoid_graph(forbidden_areas)

        # 暂时替换掉 self._graph
        original_graph = self._graph
        self._graph = avoid_graph
        try:
            # 这里显式调用类里的原版 trace_route 实现
            return GlobalRoutePlanner.trace_route(self, origin, destination)
        finally:
            # 无论成功/异常都还原
